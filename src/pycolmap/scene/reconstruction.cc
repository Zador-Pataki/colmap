     ...
     .def("find_local_bundle_ids", [](const Reconstruction& self, 
      ...
      .def("get_point3d_coordinates",
             [](const Reconstruction& self, const std::vector<point3D_t>& point_ids) -> py::array {
                 ...
                 // Iterate through the given point IDs
                 for (const auto& id : point_ids) {
                     auto it = self.Points3D().find(id);  // Access the points3D map
                     if (it != self.Points3D().end()) {
                         const auto& xyz = it->second.xyz;  // Retrieve xyz from Point3D
                 ...
                 std::vector<py::ssize_t> strides = {3 * sizeof(double), sizeof(double)};  // Strides for row-major storage

                 return py::array(py::buffer_info(
                     coordinates.data(),                // Pointer to data
