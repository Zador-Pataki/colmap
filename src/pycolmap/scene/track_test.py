...
def test_track_lc_observations_default_empty():
    track = pycolmap.Track()
    assert track.lc_observations == []


def test_track_lc_observations_roundtrip():
    track = pycolmap.Track()
    elems = [
        pycolmap.TrackElement(image_id=1, point2D_idx=5),
        pycolmap.TrackElement(image_id=2, point2D_idx=3),
    ]
    track.lc_observations = elems
    assert len(track.lc_observations) == 2
    assert track.lc_observations[0].image_id == 1
    assert track.lc_observations[1].point2D_idx == 3


def test_track_is_initialized_default_false():
    track = pycolmap.Track()
    assert track.is_initialized is False


def test_track_is_initialized_roundtrip():
    track = pycolmap.Track()
    track.is_initialized = True
    assert track.is_initialized is True
