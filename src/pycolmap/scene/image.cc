...
void BindImage(py::module& m) {
  ...
  PyImage.def(py::init<>())
      ...
      .def(
          ...
          "Get an Nx2 numpy array of xy coordinates for the specified 2D point IDs. If no IDs are provided, return all."
      )
      .def(
          "get_point3D_ids",
          ...
          "point_ids"_a = std::nullopt,
          "Get a list of 3D point IDs corresponding to the specified 2D point IDs. Returns -1 for points without a 3D point. If no IDs are provided, return for all."
      )
      .def(
          "get_point2D_objects",
          [](const Image& self, std::optional<std::vector<point2D_t>> point_ids = std::nullopt) {
              ...
              for (const auto& id : *point_ids) {
                  points2D.push_back(self.Point2D(id));
              }
              return points2D;
          },
          "point_ids"_a = std::nullopt,
          "Get a list of Point2D objects corresponding to the specified 2D point IDs. If no IDs are provided, return for all."
      )
      .def(
