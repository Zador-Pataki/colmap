...
struct MahalanobisBATADirectionalErrorWithRotation {
  MahalanobisBATADirectionalErrorWithRotation(
  ...
  static ceres::CostFunction* Create(const Eigen::Vector3d& bearing_cam,
    ...
    if (sigma_z <= 0.0) {
      LOG(ERROR)
          << "Invalid sigma_z for MahalanobisBATADirectionalErrorWithRotation: "
          << sigma_z;
      return nullptr;
    }
    // Params: rotation(4), position1(3), position2(3), scale(1)
    return new ceres::AutoDiffCostFunction<
        MahalanobisBATADirectionalErrorWithRotation, 3, 4, 3, 3, 1>(
        new MahalanobisBATADirectionalErrorWithRotation(
            bearing_cam, L00, L10, L11, sigma_z));
  }
