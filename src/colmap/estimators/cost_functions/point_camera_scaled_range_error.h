...
struct PointCameraScaledRangeError {
  ...
  bool operator()(const T* const c_i,  // Camera center (3 parameters)
                  ...
                  const T* const l_i,  // Log scale for image i (1 parameter)
                  T* residuals) const {
    ...
    // Calculate scale factor from log_scale: s_i = exp(l_i)
    T scale_factor = ceres::exp(l_i[0]);

    // Calculate scaled prior range: s_i * r_hat_ik = s_i * (m_ik / v_cam_z)
    T scaled_prior_range = scale_factor * T(depth_prior_ / v_cam_z_);

    if (scaled_prior_range < T(1e-9)) {
      throw std::runtime_error("Scaled depth prior is near zero.");
    }

    // Calculate inverse scaled prior range: 1 / (s_i * m_ik)
    T inv_scaled_prior_range = T(1.0) / scaled_prior_range;

    // Calculate target scaled vector: (X_k - c_i) / (s_i * r_hat_ik)
    Eigen::Matrix<T, 3, 1> target_scaled_vec =
        point_vec * inv_scaled_prior_range;

    // Residual = Observed_Direction - Target_Scaled_Vector, weighted
    Eigen::Map<Eigen::Matrix<T, 3, 1>> residuals_vec(residuals);
    residuals_vec = T(weight_) * (v_ik_.cast<T>() - target_scaled_vec);
  ...
  static ceres::CostFunction* Create(
    ...
    if (feature_undist_z <= 1e-9) {
      LOG(ERROR) << "Cannot create PointCameraScaledRangeError: feature_undist "
                    "Z-component is non-positive ("
                 << feature_undist_z << ")";
      return nullptr;  // Return null if invalid input
    }
    return new ceres::
        AutoDiffCostFunction<PointCameraScaledRangeError, 3, 3, 3, 1>(
            new PointCameraScaledRangeError(
                v_ik, depth_prior, feature_undist_z, depth_stddev));
  }
 ...
 private:
  ...
  const double weight_;       // Weight based on depth_stddev (1/sigma)
};
