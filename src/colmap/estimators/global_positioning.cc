...
bool GlobalPositioner::Solve(const PoseGraph& pose_graph,
...

  {
    ...
    if (const char* path = GpStateDumpPath()) {
      ...
      if (f != nullptr) {
        std::fprintf(f,
                     ...
                     static_cast<int>(solver_options.linear_solver_type),
                     static_cast<int>(solver_options.preconditioner_type),
    ...
    diagnostics_.num_point3D_xyz =
        static_cast<int>(initial_point3D_xyz_.size());
