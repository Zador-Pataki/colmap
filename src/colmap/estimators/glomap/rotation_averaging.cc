...
#include "colmap/glomap/math/tree.h"
...
// Ported from fork's glomap/math/gravity.cc — upright rotation helpers
// (identity-axis angle around Y).
inline double RotUpToAngle(const Eigen::Matrix3d& R_up) {
  Eigen::AngleAxisd aa(R_up);
  return (aa.angle() * aa.axis())[1];
}

inline Eigen::Matrix3d AngleToRotUp(double angle) {
  Eigen::Vector3d aa(0, angle, 0);
  return Eigen::AngleAxisd(aa.norm(), aa.normalized()).toRotationMatrix();
}
...
// Ceres cost functor for soft gravity alignment prior.
// Penalizes deviation of R(angle_axis) @ g_world from expected g_cam.
// 3 residuals, 1 parameter block (angle-axis, 3 elements).
// Constrains roll+pitch (2 DOF), yaw is null space.
struct GravityAlignmentError {
  ...
  bool operator()(const T* const angle_axis, T* residuals) const {
    // Convert angle-axis to rotation matrix (column-major)
    Eigen::Matrix<T, 3, 3> R;
    ceres::AngleAxisToRotationMatrix(angle_axis, R.data());

    // Predicted gravity in camera frame: R_cam_from_world @ g_world
    Eigen::Matrix<T, 3, 1> g_world_t = g_world_.cast<T>();
    Eigen::Matrix<T, 3, 1> g_pred = R * g_world_t;

    // Residual: (predicted - expected) / sigma
    for (int i = 0; i < 3; i++) {
...
RotationEstimator::EstimateRotations(
  ...
  // Video constraints are not compatible with gravity alignment
  // (different DOF counts per camera)
  ...
  // Set up the linear system (needed for both paths to initialize
  // rotation_estimated_)
  SetupLinearSystem(view_graph, images);

  // Default empty map if L1/IRLS fails early
  std::unordered_map<image_pair_t, double> final_weights;
  bool success = true;

  if (use_video_constraints) {
    // NEW PATH: Video-aware Ceres solver
    LOG(INFO) << "Using Video-Aware Ceres solver.";
    success = SolveCeres(view_graph, images);
    if (!success) {
      LOG(WARNING) << "Video-Aware Ceres solver failed.";
      return {false, {}};
    }
    // Note: SolveCeres doesn't compute edge weights for output.
    // We return empty weights map for video-aware mode.
  } else {
    // ORIGINAL PATH (Backward Compatible)
    bool l1_success = true;
    bool irls_success = true;

    // Solve the linear system for L1 norm optimization
    if (options_.max_num_l1_iterations > 0) {
      l1_success = SolveL1Regression(view_graph, images);
      if (!l1_success) {
        LOG(WARNING) << "L1 Regression failed.";
        // Return failure and empty weights
        return {false, {}};
      }
    }

    // Solve the linear system for IRLS optimization or weighted LS
    if (options_.use_precomputed_weights) {
      // Use pre-computed weights from image_pairs
      auto wls_result = SolveWeightedLS(view_graph, images);
      irls_success = wls_result.first;
      final_weights = wls_result.second;

      if (!irls_success) {
        LOG(WARNING) << "Weighted LS failed.";
        return {false, final_weights};
      }
    } else if (options_.max_num_irls_iterations > 0) {
      // Use IRLS to compute weights dynamically
      auto irls_result = SolveIRLS(view_graph, images);
      irls_success = irls_result.first;
      final_weights = irls_result.second;  // Get the weights map

      if (!irls_success) {
        LOG(WARNING) << "IRLS failed.";
        // Return failure but potentially still return the weights from the last
        // iteration
        return {false, final_weights};
      }
    }
    success = l1_success && irls_success;
  }
  ...
  for (auto& [image_id, image] : images) {
    ...
    if (options_.use_gravity && image.gravity_info.has_gravity) {
      image.cam_from_world.rotation() = Eigen::Quaterniond(  // TODO(rigid3d-sweep): manual review needed
          image.gravity_info.GetRAlign() *
          AngleToRotUp(rotation_estimated_[image_id_to_idx_[image_id]]));
    } else {
      image.cam_from_world.rotation() = Eigen::Quaterniond(AngleAxisToRotation(  // TODO(rigid3d-sweep): manual review needed
...
void RotationEstimator::InitializeFromMaximumSpanningTree(
  ...
  while (!indexes.empty()) {
    ...
    if (image_pair.image_id1 == curr) {
      // 1_R_w = 2_R_1^T * 2_R_w
      images[curr].cam_from_world.rotation() =  // TODO(rigid3d-sweep): manual review needed
...
void RotationEstimator::SetupLinearSystem(
  ...
  int skipped_risky_LC = 0;
  int skipped_unregistered = 0;
  int pairs_used = 0;
  ...
  if (skipped_risky_LC > 0) {
    LOG(INFO) << "Skipped " << skipped_risky_LC
              << " risky LC pairs (LC inliers > normal inliers)";
  }
  ...
  // Log pair usage statistics
  LOG(INFO) << "Rotation averaging pair statistics:"
            << " total=" << total_pairs << ", used=" << pairs_used
            << ", skipped_invalid=" << skipped_invalid
            << ", skipped_risky_LC=" << skipped_risky_LC
...
std::pair<bool, std::unordered_map<image_pair_t, double>>
RotationEstimator::SolveIRLS(const ViewGraph& view_graph,
  ...
  for (iteration = 0; iteration < options_.max_num_irls_iterations;
    ...
    for (auto& [pair_id, pair_info] : rel_temp_info_) {
      ...
      if (options_.weight_type == RotationEstimatorOptions::WeightType::GEMAN_MCCLURE) {
        double tmp = err_squared + sigma * sigma;
        w = sigma * sigma / (tmp * tmp);
      } else if (options_.weight_type == RotationEstimatorOptions::WeightType::HALF_NORM) {
...
std::pair<bool, std::unordered_map<image_pair_t, double>>
RotationEstimator::SolveWeightedLS(const ViewGraph& view_graph,
                                   std::unordered_map<image_t, Image>& images) {
  std::unordered_map<image_pair_t, double> final_weights;

  // Use pre-computed weights from image_pairs
  LOG(INFO) << "Using pre-computed weights from image_pairs for weighted least "
               "squares";
  ...
  for (const auto& [pair_id, pair_info] : rel_temp_info_) {
    ...
    const ImagePair& image_pair = view_graph.image_pairs.at(pair_id);
...
bool RotationEstimator::SolveCeres(const ViewGraph& view_graph,
                                   std::unordered_map<image_t, Image>& images) {
  ...
  // Add soft gravity prior constraints
  if (options_.use_gravity_prior) {
    int gravity_count = 0;
    for (auto& [image_id, image] : images) {
      if (!image.is_registered) continue;
      if (!image.gravity_info.has_gravity) continue;
      if (image_id == fixed_camera_id_) continue;

      Eigen::Vector3d g_cam = image.gravity_info.GetGravity();
      double sigma = image.gravity_sigma > 0
                         ? image.gravity_sigma
                         : options_.default_gravity_sigma;

      ceres::CostFunction* grav_cost = GravityAlignmentError::Create(
          g_cam, options_.gravity_world, 1.0 / sigma);

      ceres::LossFunction* grav_loss = nullptr;
      if (options_.gravity_loss_type == "cauchy") {
        grav_loss = new ceres::CauchyLoss(options_.gravity_loss_scale);
      } else if (options_.gravity_loss_type == "huber") {
        grav_loss = new ceres::HuberLoss(options_.gravity_loss_scale);
      }
      // "trivial" or anything else: grav_loss stays nullptr (no robustification)

      problem.AddResidualBlock(
          grav_cost, grav_loss,
          rotation_estimated_.data() + image_id_to_idx_[image_id]);
      gravity_count++;
    }
    LOG(INFO) << "Added " << gravity_count << " gravity prior constraints.";
  }
  ...
  LOG(INFO) << "Video-Aware Ceres Solver: " << summary.BriefReport();
