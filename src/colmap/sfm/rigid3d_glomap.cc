...
Eigen::Vector3d Rigid3dToAngleAxis(const Rigid3d& pose) {
  // Materialize the Eigen::Map<Quaternion> into a real Quaternion first; the
  // AngleAxis ctor accepts QuaternionBase but not the Map alias directly.
  Eigen::AngleAxis<double> aa(Eigen::Quaterniond(pose.rotation()));
