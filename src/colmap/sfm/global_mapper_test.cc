...
TEST(GlobalMapper, SolveAppliesTrackSubsampleOptions) {
  ...
  EXPECT_LE(reconstruction->NumPoints3D(), options.track_max_num_tracks);
}

TEST(GlobalMapper,
     EstablishTracksDefersRequiredTracksPerViewToProblemSubsample) {
  auto reconstruction = std::make_shared<Reconstruction>();
  ...
  global_mapper.SubsampleTracksForProblem(options);
