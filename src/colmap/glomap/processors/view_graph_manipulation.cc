...
image_pair_t ViewGraphManipulater::SparsifyGraph(
  ...
  if (num_img > 0) average_degree = average_degree / num_img;
  ...
  for (auto& [pair_id, image_pair] : view_graph.image_pairs) {
    ...
    if (!images[image_id1].is_registered || !images[image_id2].is_registered)
      continue;
...
image_t ViewGraphManipulater::EstablishStrongClusters(
  ...
  LOG(INFO) << "Clustering took " << iteration << " iterations. "
            << num_comp << " clusters after strong-clustering";
...
void ViewGraphManipulater::UpdateImagePairsConfig(
  ...
  for (auto& [pair_id, image_pair] : view_graph.image_pairs) {
    ...
    if (!camera1.camera.has_prior_focal_length ||
        !camera2.camera.has_prior_focal_length)
      continue;
...
void ViewGraphManipulater::DecomposeRelPose(
  ...
  for (auto& [pair_id, image_pair] : view_graph.image_pairs) {
    ...
    if (!cameras[images[image_pair.image_id1].camera_id]
        ...
        !cameras[images[image_pair.image_id2].camera_id]
             .camera.has_prior_focal_length)
      continue;
  ...
  for (int64_t idx = 0; idx < num_image_pairs; idx++) {
    thread_pool.AddTask([&, idx]() {
      ...
      colmap::EstimateTwoViewGeometryPose(cameras[camera_id1].camera,
                                          images[image_id1].features,
                                          cameras[camera_id2].camera,
                                          images[image_id2].features,
                                          &two_view_geometry);

      if (image_pair.config == colmap::TwoViewGeometry::PLANAR &&
          cameras[camera_id1].camera.has_prior_focal_length &&
          cameras[camera_id2].camera.has_prior_focal_length) {
        image_pair.config = colmap::TwoViewGeometry::CALIBRATED;
        return;
      } else if (!(cameras[camera_id1].camera.has_prior_focal_length &&
                   cameras[camera_id2].camera.has_prior_focal_length))
        return;
