...
image_t MaximumSpanningTree(const ViewGraph& view_graph,
  ...
  for (auto& [pair_id, image_pair] : view_graph.image_pairs) {
    ...
    // Calculate a penalty for non-tracking pairs if prioritize_tracking is enabled
    double weight_penalty = 0.0;
    if (prioritize_tracking && !image_pair.inliers.empty()) {
      // Check if this is a tracking pair (normal inliers >= LC inliers)
      int lc_count = 0;
      ...
      int normal_inliers_count =
          static_cast<int>(image_pair.inliers.size()) - lc_count;
      // If not a tracking pair (LC inliers > normal inliers), apply huge penalty
      if (lc_count > normal_inliers_count) {
        weight_penalty = 1e9;
      }
    }
    ...
    if (type == INLIER_NUM)
      weights_boost[e] = max_weight - image_pair.inliers.size() + weight_penalty;
    else if (type == INLIER_RATIO)
      weights_boost[e] = max_weight - image_pair.weight + weight_penalty;
    else
      weights_boost[e] = max_weight - image_pair.inliers.size() + weight_penalty;
  }
