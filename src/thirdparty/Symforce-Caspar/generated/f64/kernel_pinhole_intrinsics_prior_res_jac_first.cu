#include "kernel_pinhole_intrinsics_prior_res_jac_first.h"
#include "memops.cuh"
...
__global__ void __launch_bounds__(1024, 1)
    PinholeIntrinsicsPriorResJacFirstKernel(
        double* calib,
  ...
  if (global_thread_idx < problem_size) {
    ReadShared2<double>(
        (double*)inout_shared, calib_indices_loc[threadIdx.x].target, r9, r10);
  };
  __syncthreads();
  if (global_thread_idx < problem_size) {
    ReadIdx2<1024, double, double, double2>(
...
void PinholeIntrinsicsPriorResJacFirst(
    double* calib,
    unsigned int calib_num_alloc,
    SharedIndex* calib_indices,
    double* prior,
  ...
  PinholeIntrinsicsPriorResJacFirstKernel<<<n_blocks, 1024>>>(
      calib,
      calib_num_alloc,
      calib_indices,
      prior,
