...
__global__ void __launch_bounds__(1024, 1) PinholeTranslationUpdateRFirstKernel(
    ...
    double* const out_PinholeTranslation_r_0_norm2_tot,
    double* const out_PinholeTranslation_r_kp1_norm2_tot,
    size_t problem_size) {
  const int global_thread_idx = blockIdx.x * blockDim.x + threadIdx.x;
  __shared__ uint8_t inout_shared[8192];

  __shared__ double out_PinholeTranslation_r_0_norm2_tot_local[1];

  __shared__ double out_PinholeTranslation_r_kp1_norm2_tot_local[1];
  ...
  if (global_thread_idx < problem_size) {
    ...
    WriteIdx1<1024, double, double, double>(
        out_PinholeTranslation_r_kp1,
        2 * out_PinholeTranslation_r_kp1_num_alloc,
        global_thread_idx,
        r4);
    r5 = fma(r5, r5, r1 * r1);
    r5 = fma(r0, r0, r5);
  };
  SumStore<double>(out_PinholeTranslation_r_0_norm2_tot_local,
                   (double*)inout_shared,
                   0,
  ...
  SumStore<double>(out_PinholeTranslation_r_kp1_norm2_tot_local,
                   (double*)inout_shared,
                   0,
                   global_thread_idx < problem_size,
                   r3);
  SumFlushFinal<double>(out_PinholeTranslation_r_0_norm2_tot_local,
                        out_PinholeTranslation_r_0_norm2_tot,
                        1);
  SumFlushFinal<double>(out_PinholeTranslation_r_kp1_norm2_tot_local,
