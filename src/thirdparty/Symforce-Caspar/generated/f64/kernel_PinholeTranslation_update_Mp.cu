#include "kernel_PinholeTranslation_update_Mp.h"
#include "memops.cuh"
#include <cooperative_groups.h>
#include <cooperative_groups/details/partitioning.h>
#include <cooperative_groups/memcpy_async.h>
#include <cooperative_groups/reduce.h>
#include <cuda_runtime.h>

namespace cg = cooperative_groups;

namespace caspar {

__global__ void __launch_bounds__(1024, 1) PinholeTranslationUpdateMpKernel(
    double* PinholeTranslation_r_k,
    unsigned int PinholeTranslation_r_k_num_alloc,
    double* PinholeTranslation_Mp,
    unsigned int PinholeTranslation_Mp_num_alloc,
    const double* const beta,
    double* out_PinholeTranslation_Mp_kp1,
    unsigned int out_PinholeTranslation_Mp_kp1_num_alloc,
    double* out_PinholeTranslation_w,
    unsigned int out_PinholeTranslation_w_num_alloc,
    size_t problem_size) {
  const int global_thread_idx = blockIdx.x * blockDim.x + threadIdx.x;
  __shared__ uint8_t inout_shared[8192];

  double r0, r1, r2, r3, r4;

  if (global_thread_idx < problem_size) {
    ReadIdx2<1024, double, double, double2>(PinholeTranslation_Mp,
                                            0 * PinholeTranslation_Mp_num_alloc,
                                            global_thread_idx,
                                            r0,
                                            r1);
    ReadIdx2<1024, double, double, double2>(
        PinholeTranslation_r_k,
        0 * PinholeTranslation_r_k_num_alloc,
        global_thread_idx,
        r2,
        r3);
  };
  LoadUnique<1, double, double>(beta, 0, (double*)inout_shared);
  if (global_thread_idx < problem_size) {
    ReadShared1<double>((double*)inout_shared, 0, r4);
  };
  __syncthreads();
  if (global_thread_idx < problem_size) {
    r0 = fma(r0, r4, r2);
    r1 = fma(r1, r4, r3);
    WriteIdx2<1024, double, double, double2>(
        out_PinholeTranslation_Mp_kp1,
        0 * out_PinholeTranslation_Mp_kp1_num_alloc,
        global_thread_idx,
        r0,
        r1);
    ReadIdx1<1024, double, double, double>(PinholeTranslation_Mp,
                                           2 * PinholeTranslation_Mp_num_alloc,
                                           global_thread_idx,
                                           r3);
    ReadIdx1<1024, double, double, double>(PinholeTranslation_r_k,
                                           2 * PinholeTranslation_r_k_num_alloc,
                                           global_thread_idx,
                                           r2);
    r4 = fma(r3, r4, r2);
    WriteIdx1<1024, double, double, double>(
        out_PinholeTranslation_Mp_kp1,
        2 * out_PinholeTranslation_Mp_kp1_num_alloc,
        global_thread_idx,
        r4);
    WriteIdx2<1024, double, double, double2>(
        out_PinholeTranslation_w,
        0 * out_PinholeTranslation_w_num_alloc,
        global_thread_idx,
        r0,
        r1);
    WriteIdx1<1024, double, double, double>(
        out_PinholeTranslation_w,
        2 * out_PinholeTranslation_w_num_alloc,
        global_thread_idx,
        r4);
  };
}

void PinholeTranslationUpdateMp(
    double* PinholeTranslation_r_k,
    unsigned int PinholeTranslation_r_k_num_alloc,
    double* PinholeTranslation_Mp,
    unsigned int PinholeTranslation_Mp_num_alloc,
    const double* const beta,
    double* out_PinholeTranslation_Mp_kp1,
    unsigned int out_PinholeTranslation_Mp_kp1_num_alloc,
    double* out_PinholeTranslation_w,
    unsigned int out_PinholeTranslation_w_num_alloc,
    size_t problem_size) {
  if (problem_size == 0) {
    return;
  }

  const int n_blocks = (problem_size + 1024 - 1) / 1024;
  PinholeTranslationUpdateMpKernel<<<n_blocks, 1024>>>(
      PinholeTranslation_r_k,
      PinholeTranslation_r_k_num_alloc,
      PinholeTranslation_Mp,
      PinholeTranslation_Mp_num_alloc,
      beta,
      out_PinholeTranslation_Mp_kp1,
      out_PinholeTranslation_Mp_kp1_num_alloc,
      out_PinholeTranslation_w,
      out_PinholeTranslation_w_num_alloc,
      problem_size);
}

}  // namespace caspar