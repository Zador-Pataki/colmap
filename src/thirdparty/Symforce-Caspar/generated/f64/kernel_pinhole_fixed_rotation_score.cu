#include "kernel_pinhole_fixed_rotation_score.h"
#include "memops.cuh"
...
__global__ void __launch_bounds__(1024, 1)
    PinholeFixedRotationScoreKernel(double* rotation,
  ...
  if (global_thread_idx < problem_size) {
    ReadShared2<double>(
        (double*)inout_shared, calib_indices_loc[threadIdx.x].target, r3, r7);
  };
  __syncthreads();
...
void PinholeFixedRotationScore(double* rotation,
                               unsigned int rotation_num_alloc,
                               double* translation,
                               unsigned int translation_num_alloc,
                               SharedIndex* translation_indices,
  ...
  PinholeFixedRotationScoreKernel<<<n_blocks, 1024>>>(rotation,
                                                      rotation_num_alloc,
                                                      translation,
