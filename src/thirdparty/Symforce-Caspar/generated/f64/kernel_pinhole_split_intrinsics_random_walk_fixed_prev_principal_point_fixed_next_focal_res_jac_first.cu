#include "kernel_pinhole_split_intrinsics_random_walk_fixed_prev_principal_point_fixed_next_focal_res_jac_first.h"
#include "memops.cuh"
...
__global__ void __launch_bounds__(1024, 1)
    PinholeSplitIntrinsicsRandomWalkFixedPrevPrincipalPointFixedNextFocalResJacFirstKernel(
        double* prev_focal,
  ...
  if (global_thread_idx < problem_size) {
    ...
    WriteIdx2<1024, double, double, double2>(out_prev_focal_jac,
                                             ...
                                             r0,
                                             r1);
    WriteSum2<double, double>((double*)inout_shared, r11, r13);
  };
  FlushSumShared<2, double>(out_prev_focal_njtr,
                            ...
                            (double*)inout_shared);
  if (global_thread_idx < problem_size) {
    WriteSum2<double, double>((double*)inout_shared, r9, r12);
  };
...
void PinholeSplitIntrinsicsRandomWalkFixedPrevPrincipalPointFixedNextFocalResJacFirst(
    double* prev_focal,
    unsigned int prev_focal_num_alloc,
    SharedIndex* prev_focal_indices,
    double* next_principal_point,
