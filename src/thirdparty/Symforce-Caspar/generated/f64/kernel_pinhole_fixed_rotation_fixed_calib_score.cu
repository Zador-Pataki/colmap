#include "kernel_pinhole_fixed_rotation_fixed_calib_score.h"
#include "memops.cuh"
...
__global__ void __launch_bounds__(1024, 1)
    PinholeFixedRotationFixedCalibScoreKernel(
        double* rotation,
  ...
  if (global_thread_idx < problem_size) {
    ReadIdx2<1024, double, double, double2>(
        weight_loss, 2 * weight_loss_num_alloc, global_thread_idx, r0, r1);
    ReadIdx2<1024, double, double, double2>(
    ...
    r5 = fma(r5, r6, r3);
    ReadIdx2<1024, double, double, double2>(
        calib, 0 * calib_num_alloc, global_thread_idx, r3, r7);
  };
  ...
  if (global_thread_idx < problem_size) {
    ...
    r12 = sqrt(r1);
    ReadIdx2<1024, double, double, double2>(
        weight_loss, 0 * weight_loss_num_alloc, global_thread_idx, r11, r22);
...
void PinholeFixedRotationFixedCalibScore(double* rotation,
                                         unsigned int rotation_num_alloc,
                                         double* translation,
                                         unsigned int translation_num_alloc,
                                         SharedIndex* translation_indices,
  ...
  PinholeFixedRotationFixedCalibScoreKernel<<<n_blocks, 1024>>>(
      rotation,
      rotation_num_alloc,
      translation,
