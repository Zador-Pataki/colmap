#include "kernel_pinhole_split_intrinsics_prior_fixed_principal_point_score.h"
#include "memops.cuh"
...
__global__ void __launch_bounds__(1024, 1)
    PinholeSplitIntrinsicsPriorFixedPrincipalPointScoreKernel(
        double* focal,
  ...
  if (global_thread_idx < problem_size) {
    ReadShared2<double>(
        (double*)inout_shared, focal_indices_loc[threadIdx.x].target, r3, r0);
  };
  __syncthreads();
  if (global_thread_idx < problem_size) {
    ReadIdx2<1024, double, double, double2>(
...
void PinholeSplitIntrinsicsPriorFixedPrincipalPointScore(
  ...
  PinholeSplitIntrinsicsPriorFixedPrincipalPointScoreKernel<<<n_blocks, 1024>>>(
      focal,
      focal_num_alloc,
      focal_indices,
      prior,
