#include "kernel_pinhole_split_intrinsics_random_walk_fixed_prev_principal_point_jtjnjtr_direct.h"
#include "memops.cuh"
...
__global__ void __launch_bounds__(1024, 1)
    PinholeSplitIntrinsicsRandomWalkFixedPrevPrincipalPointJtjnjtrDirectKernel(
  ...
  LoadShared<2, double, double>(next_focal_njtr,
  ...
  __syncthreads();
  if (global_thread_idx < problem_size) {
    ReadIdx2<1024, double, double, double2>(prev_focal_jac,
                                            0 * prev_focal_jac_num_alloc,
                                            global_thread_idx,
                                            r2,
                                            r3);
    ReadIdx2<1024, double, double, double2>(next_focal_jac,
  ...
  FlushSumShared<2, double>(out_prev_focal_njtr,
                            ...
                            (double*)inout_shared);
  LoadShared<2, double, double>(prev_focal_njtr,
...
void PinholeSplitIntrinsicsRandomWalkFixedPrevPrincipalPointJtjnjtrDirect(
    double* prev_focal_njtr,
    unsigned int prev_focal_njtr_num_alloc,
    SharedIndex* prev_focal_njtr_indices,
    double* prev_focal_jac,
