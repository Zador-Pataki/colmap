#include "kernel_pinhole_log_depth_fixed_pose_fixed_point_score.h"
#include "memops.cuh"
...
__global__ void __launch_bounds__(1024, 1)
    PinholeLogDepthFixedPoseFixedPointScoreKernel(
        double* scale,
  ...
  if (global_thread_idx < problem_size) {
    ...
    r6 = fma(r7, r1, r4 * r6);
    ReadIdx1<1024, double, double, double>(
        point, 2 * point_num_alloc, global_thread_idx, r10);
  ...
  if (global_thread_idx < problem_size) {
    ReadShared1<double>(
        (double*)inout_shared, scale_indices_loc[threadIdx.x].target, r1);
  };
  __syncthreads();
  if (global_thread_idx < problem_size) {
...
void PinholeLogDepthFixedPoseFixedPointScore(double* scale,
                                             unsigned int scale_num_alloc,
                                             SharedIndex* scale_indices,
                                             double* log_depth,
                                             unsigned int log_depth_num_alloc,
  ...
  PinholeLogDepthFixedPoseFixedPointScoreKernel<<<n_blocks, 1024>>>(
      scale,
      scale_num_alloc,
      scale_indices,
      log_depth,
