#include "kernel_DepthScale_update_step_first.h"
#include "memops.cuh"
#include <cooperative_groups.h>
#include <cooperative_groups/details/partitioning.h>
#include <cooperative_groups/memcpy_async.h>
#include <cooperative_groups/reduce.h>
#include <cuda_runtime.h>

namespace cg = cooperative_groups;

namespace caspar {

__global__ void __launch_bounds__(1024, 1) DepthScaleUpdateStepFirstKernel(
    double* DepthScale_p_kp1,
    unsigned int DepthScale_p_kp1_num_alloc,
    const double* const alpha,
    double* out_DepthScale_step_kp1,
    unsigned int out_DepthScale_step_kp1_num_alloc,
    size_t problem_size) {
  const int global_thread_idx = blockIdx.x * blockDim.x + threadIdx.x;
  __shared__ uint8_t inout_shared[8192];

  double r0, r1;

  if (global_thread_idx < problem_size) {
    ReadIdx1<1024, double, double, double>(DepthScale_p_kp1,
                                           0 * DepthScale_p_kp1_num_alloc,
                                           global_thread_idx,
                                           r0);
  };
  LoadUnique<1, double, double>(alpha, 0, (double*)inout_shared);
  if (global_thread_idx < problem_size) {
    ReadShared1<double>((double*)inout_shared, 0, r1);
  };
  __syncthreads();
  if (global_thread_idx < problem_size) {
    r1 = r0 * r1;
    WriteIdx1<1024, double, double, double>(
        out_DepthScale_step_kp1,
        0 * out_DepthScale_step_kp1_num_alloc,
        global_thread_idx,
        r1);
  };
}

void DepthScaleUpdateStepFirst(double* DepthScale_p_kp1,
                               unsigned int DepthScale_p_kp1_num_alloc,
                               const double* const alpha,
                               double* out_DepthScale_step_kp1,
                               unsigned int out_DepthScale_step_kp1_num_alloc,
                               size_t problem_size) {
  if (problem_size == 0) {
    return;
  }

  const int n_blocks = (problem_size + 1024 - 1) / 1024;
  DepthScaleUpdateStepFirstKernel<<<n_blocks, 1024>>>(
      DepthScale_p_kp1,
      DepthScale_p_kp1_num_alloc,
      alpha,
      out_DepthScale_step_kp1,
      out_DepthScale_step_kp1_num_alloc,
      problem_size);
}

}  // namespace caspar