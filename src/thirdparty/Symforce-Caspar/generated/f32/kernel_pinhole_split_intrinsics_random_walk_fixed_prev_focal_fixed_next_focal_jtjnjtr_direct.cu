#include "kernel_pinhole_split_intrinsics_random_walk_fixed_prev_focal_fixed_next_focal_jtjnjtr_direct.h"
#include "memops.cuh"
...
__global__ void __launch_bounds__(1024, 1)
    PinholeSplitIntrinsicsRandomWalkFixedPrevFocalFixedNextFocalJtjnjtrDirectKernel(
  ...
  LoadShared<2, float, float>(next_principal_point_njtr,
  ...
  __syncthreads();
  if (global_thread_idx < problem_size) {
    ReadIdx2<1024, float, float, float2>(prev_principal_point_jac,
                                         0 * prev_principal_point_jac_num_alloc,
                                         global_thread_idx,
                                         r2,
                                         r3);
    ReadIdx2<1024, float, float, float2>(next_principal_point_jac,
  ...
  FlushSumShared<2, float>(out_prev_principal_point_njtr,
                           ...
                           (float*)inout_shared);
  LoadShared<2, float, float>(prev_principal_point_njtr,
...
void PinholeSplitIntrinsicsRandomWalkFixedPrevFocalFixedNextFocalJtjnjtrDirect(
    float* prev_principal_point_njtr,
    unsigned int prev_principal_point_njtr_num_alloc,
    SharedIndex* prev_principal_point_njtr_indices,
    float* prev_principal_point_jac,
  ...
  PinholeSplitIntrinsicsRandomWalkFixedPrevFocalFixedNextFocalJtjnjtrDirectKernel<<<
      n_blocks,
      1024>>>(prev_principal_point_njtr,
