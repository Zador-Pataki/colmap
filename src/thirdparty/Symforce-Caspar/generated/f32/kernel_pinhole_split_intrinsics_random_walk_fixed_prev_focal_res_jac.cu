#include "kernel_pinhole_split_intrinsics_random_walk_fixed_prev_focal_res_jac.h"
#include "memops.cuh"
...
__global__ void __launch_bounds__(1024, 1)
    PinholeSplitIntrinsicsRandomWalkFixedPrevFocalResJacKernel(
  ...
  LoadShared<2, float, float>(next_focal,
  ...
  if (global_thread_idx < problem_size) {
    ...
    WriteIdx4<1024, float, float, float4>(
        out_res, 0 * out_res_num_alloc, global_thread_idx, r6, r7, r5, r4);
    r4 = r2 * r8;
    r5 = r3 * r8;
  ...
  FlushSumShared<2, float>(out_prev_principal_point_njtr,
                           ...
                           (float*)inout_shared);
  if (global_thread_idx < problem_size) {
    WriteSum2<float, float>((float*)inout_shared, r5, r4);
  };
  FlushSumShared<2, float>(out_prev_principal_point_precond_diag,
                           ...
                           (float*)inout_shared);
  if (global_thread_idx < problem_size) {
    WriteIdx2<1024, float, float, float2>(out_next_focal_jac,
    ...
    r6 = r1 * r8;
    r6 = r6 * r7;
    WriteSum2<float, float>((float*)inout_shared, r11, r6);
  };
  ...
  FlushSumShared<2, float>(out_next_focal_precond_diag,
...
void PinholeSplitIntrinsicsRandomWalkFixedPrevFocalResJac(
    float* prev_principal_point,
    unsigned int prev_principal_point_num_alloc,
    SharedIndex* prev_principal_point_indices,
    float* next_focal,
  ...
  PinholeSplitIntrinsicsRandomWalkFixedPrevFocalResJacKernel<<<n_blocks,
                                                               1024>>>(
