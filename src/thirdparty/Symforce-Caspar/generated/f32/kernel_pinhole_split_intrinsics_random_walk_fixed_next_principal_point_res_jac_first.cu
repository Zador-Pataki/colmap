#include "kernel_pinhole_split_intrinsics_random_walk_fixed_next_principal_point_res_jac_first.h"
#include "memops.cuh"
...
__global__ void __launch_bounds__(1024, 1)
    PinholeSplitIntrinsicsRandomWalkFixedNextPrincipalPointResJacFirstKernel(
        float* prev_focal,
  ...
  if (global_thread_idx < problem_size) {
    ...
    WriteIdx2<1024, float, float, float2>(out_prev_focal_jac,
                                          ...
                                          r12,
                                          r7);
    WriteSum2<float, float>((float*)inout_shared, r13, r15);
  };
  FlushSumShared<2, float>(out_prev_focal_njtr,
                           ...
                           (float*)inout_shared);
  if (global_thread_idx < problem_size) {
    WriteSum2<float, float>((float*)inout_shared, r11, r14);
  };
  ...
  if (global_thread_idx < problem_size) {
    ...
    WriteIdx2<1024, float, float, float2>(
        ...
        r2,
        r3);
    WriteSum2<float, float>((float*)inout_shared, r9, r4);
  };
...
void PinholeSplitIntrinsicsRandomWalkFixedNextPrincipalPointResJacFirst(
    float* prev_focal,
    unsigned int prev_focal_num_alloc,
    SharedIndex* prev_focal_indices,
    float* prev_principal_point,
