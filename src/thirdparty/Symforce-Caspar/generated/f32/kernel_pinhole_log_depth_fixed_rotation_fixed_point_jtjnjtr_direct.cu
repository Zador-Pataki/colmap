#include "kernel_pinhole_log_depth_fixed_rotation_fixed_point_jtjnjtr_direct.h"
#include "memops.cuh"
...
__global__ void __launch_bounds__(1024, 1)
    PinholeLogDepthFixedRotationFixedPointJtjnjtrDirectKernel(
  ...
  if (global_thread_idx < problem_size) {
    ReadShared1<float>(
        (float*)inout_shared, scale_njtr_indices_loc[threadIdx.x].target, r1);
  };
  __syncthreads();
  if (global_thread_idx < problem_size) {
    ReadIdx1<1024, float, float, float>(
        translation_jac, 0 * translation_jac_num_alloc, global_thread_idx, r2);
    ReadIdx1<1024, float, float, float>(
...
void PinholeLogDepthFixedRotationFixedPointJtjnjtrDirect(
    float* translation_njtr,
    unsigned int translation_njtr_num_alloc,
    SharedIndex* translation_njtr_indices,
    float* translation_jac,
  ...
  PinholeLogDepthFixedRotationFixedPointJtjnjtrDirectKernel<<<n_blocks, 1024>>>(
      translation_njtr,
