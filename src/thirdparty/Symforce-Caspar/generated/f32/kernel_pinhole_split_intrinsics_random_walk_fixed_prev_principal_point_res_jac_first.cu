#include "kernel_pinhole_split_intrinsics_random_walk_fixed_prev_principal_point_res_jac_first.h"
#include "memops.cuh"
...
__global__ void __launch_bounds__(1024, 1)
    PinholeSplitIntrinsicsRandomWalkFixedPrevPrincipalPointResJacFirstKernel(
        float* prev_focal,
  ...
  if (global_thread_idx < problem_size) {
    ...
    WriteIdx2<1024, float, float, float2>(out_prev_focal_jac,
                                          ...
                                          r12,
                                          r7);
    WriteSum2<float, float>((float*)inout_shared, r5, r13);
  };
  FlushSumShared<2, float>(out_prev_focal_njtr,
                           ...
                           (float*)inout_shared);
  if (global_thread_idx < problem_size) {
    WriteSum2<float, float>((float*)inout_shared, r11, r4);
  };
...
void PinholeSplitIntrinsicsRandomWalkFixedPrevPrincipalPointResJacFirst(
    float* prev_focal,
    unsigned int prev_focal_num_alloc,
    SharedIndex* prev_focal_indices,
    float* next_focal,
