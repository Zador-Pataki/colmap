#include "kernel_pinhole_split_intrinsics_random_walk_fixed_prev_focal_fixed_next_focal_res_jac.h"
#include "memops.cuh"
...
__global__ void __launch_bounds__(1024, 1)
    PinholeSplitIntrinsicsRandomWalkFixedPrevFocalFixedNextFocalResJacKernel(
  ...
  if (global_thread_idx < problem_size) {
    ...
    WriteIdx4<1024, float, float, float4>(
        out_res, 0 * out_res_num_alloc, global_thread_idx, r6, r7, r1, r5);
    r5 = r2 * r8;
    r1 = r3 * r8;
    ...
    r5 = r3 * r3;
    r4 = r4 * r5;
    WriteSum2<float, float>((float*)inout_shared, r0, r4);
  };
  FlushSumShared<2, float>(out_prev_principal_point_njtr,
                           ...
                           (float*)inout_shared);
  if (global_thread_idx < problem_size) {
    WriteSum2<float, float>((float*)inout_shared, r1, r5);
  };
...
void PinholeSplitIntrinsicsRandomWalkFixedPrevFocalFixedNextFocalResJac(
    float* prev_principal_point,
    unsigned int prev_principal_point_num_alloc,
    SharedIndex* prev_principal_point_indices,
    float* next_principal_point,
