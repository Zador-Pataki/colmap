...
__global__ void __launch_bounds__(1024, 1)
    DepthScaleUpdateRFirstKernel(float* DepthScale_r_k,
                                 ...
                                 float* const out_DepthScale_r_0_norm2_tot,
                                 float* const out_DepthScale_r_kp1_norm2_tot,
                                 size_t problem_size) {
  const int global_thread_idx = blockIdx.x * blockDim.x + threadIdx.x;
  __shared__ uint8_t inout_shared[4096];

  __shared__ float out_DepthScale_r_0_norm2_tot_local[1];

  __shared__ float out_DepthScale_r_kp1_norm2_tot_local[1];
  ...
  if (global_thread_idx < problem_size) {
    r2 = fmaf(r1, r2, r0);
    WriteIdx1<1024, float, float, float>(out_DepthScale_r_kp1,
                                         0 * out_DepthScale_r_kp1_num_alloc,
                                         global_thread_idx,
                                         r2);
    r0 = r0 * r0;
  };
  SumStore<float>(out_DepthScale_r_0_norm2_tot_local,
                  (float*)inout_shared,
                  0,
  ...
  SumStore<float>(out_DepthScale_r_kp1_norm2_tot_local,
                  (float*)inout_shared,
                  0,
                  global_thread_idx < problem_size,
                  r2);
...
void DepthScaleUpdateRFirst(float* DepthScale_r_k,
                            unsigned int DepthScale_r_k_num_alloc,
                            float* DepthScale_w,
                            unsigned int DepthScale_w_num_alloc,
                            const float* const negalpha,
