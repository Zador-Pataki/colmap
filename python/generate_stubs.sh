...
$PYTHON_EXEC -m pybind11_stubgen $PACKAGE_NAME -o $OUTPUT \
        ...
        --ignore-invalid-expressions "(ceres|std)::.*" \
...
# Pybind11 sometimes prints raw std::unordered_map<> for properties returning
# bound maps (workaround for a typedef-name resolution quirk). Map them back
# to the registered Python alias.
perl -i -pe 's/-> std::unordered_map<.*ImagePair[^:]*>:/-> ImagePairMap:/g' $FILES
