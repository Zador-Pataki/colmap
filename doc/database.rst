...
The ``is_loop_closure`` column stores optional pair provenance for global
mapping: ``0`` means a regular/local pair and ``1`` means at least one verified
inlier was generated by a loop-closure path. The optional
``inlier_matches_are_lc`` blob stores one byte per inlier match for
mixed-provenance pairs. When this blob is empty or ``NULL``,
``is_loop_closure`` applies to all inlier matches.
